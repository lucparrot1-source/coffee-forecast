...
def test_get_current_regime_respects_max_date(mem_conn: sqlite3.Connection) -> None:
    """get_current_regime with max_date must use only data up to that cutoff."""
    # Seed 24 months of very low-vol prices (regimes computed on 12-month rolling window)
    rng = np.random.default_rng(99)
    stable_dates = pd.date_range("2018-01-01", periods=24, freq="MS")
    stable_prices = 150.0 * np.exp(np.cumsum(rng.normal(0, 0.002, 24)))  # very low vol

    # Seed 24 months of extremely high-vol prices after the cutoff
    high_vol_dates = pd.date_range("2020-01-01", periods=24, freq="MS")
    high_vol_prices = stable_prices[-1] * np.exp(np.cumsum(rng.normal(0, 0.30, 24)))  # very high vol

    all_dates = list(stable_dates) + list(high_vol_dates)
    all_prices = list(stable_prices) + list(high_vol_prices)
    mem_conn.executemany(
        "INSERT INTO prices_monthly (date, symbol, adj_close) VALUES (?, ?, ?)",
        [(d.strftime("%Y-%m-%d"), "KC=F", float(p)) for d, p in zip(all_dates, all_prices)],
    )
    mem_conn.commit()

    # Full data → high-vol tail dominates → expect "High"
    regime_full = get_current_regime(mem_conn, max_date=None)
    assert regime_full == "High"

    # Cutoff at 2019-12 (before high-vol period) → uses only 24 stable months → expect "Low"
    regime_cut = get_current_regime(mem_conn, max_date="2019-12-01")
    assert regime_cut == "Low"


def test_run_hybrid_model_passes_max_date_to_regime(mem_conn: sqlite3.Connection) -> None:
    """run_hybrid_model with max_date must thread the cutoff through to regime computation."""
    vecm_run_id, gamlss_run_id = _seed_model_runs(mem_conn)
    _seed_vecm_forecasts(mem_conn, vecm_run_id)
    _seed_gamlss_params(mem_conn, gamlss_run_id)
    _seed_kc_prices(mem_conn, n=48)

    cutoff = pd.date_range("2020-01-01", periods=24, freq="MS")[-1].strftime("%Y-%m-%d")
    run_id = run_hybrid_model(mem_conn, vecm_run_id, gamlss_run_id, max_date=cutoff)

    assert run_id > 0
    row = mem_conn.execute(
        "SELECT status, params FROM model_runs WHERE id = ?", (run_id,)
    ).fetchone()
    assert row[0] == "success"

    # Verify the regime stored in params matches what get_current_regime returns for the same cutoff
    params = json.loads(row[1]) if row[1] else {}
    expected_regime = get_current_regime(mem_conn, max_date=cutoff)
    if "regime" in params:
