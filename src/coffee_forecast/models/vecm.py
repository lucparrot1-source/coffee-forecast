...
def load_aligned_data(
    ...
    # Ensure all expected symbols are columns (missing ones → all-NaN → dropped by dropna)
...
def select_lag_order(endog: pd.DataFrame, exog: pd.DataFrame, maxlags: int = 12) -> int:
    ...
    # Cap maxlags so select_order has enough observations (needs > maxlags+1 rows)
    effective_maxlags = min(maxlags, max(1, len(endog) // 4))
    res = select_order(endog.values, maxlags=effective_maxlags, deterministic="co", exog=exog.values)
...
def write_forecasts(
    ...
    for _, row in forecasts_df.iterrows():
        h = int(row["horizon"])
        # Normalize to YYYY-MM-01 — must match prices_monthly date convention
        # so load_actuals in backtest.py can join on date strings correctly.
        target_date = (
...
def run_vecm_model(conn: sqlite3.Connection, max_date: str | None = None) -> int:
    ...
    if endog.empty:
        ...
        return -1
    if len(endog) < 20:
        log.warning("Insufficient aligned data (%d months) — need ≥20 for VECM fit", len(endog))
        return -1
