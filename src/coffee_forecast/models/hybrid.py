...
def get_current_regime(conn: sqlite3.Connection, max_date: str | None = None) -> str:
    """Return the volatility regime label for the latest available KC=F month up to max_date.
    ...
    """
    regime_series = compute_regime_labels(conn, max_date=max_date)
...
def write_hybrid_forecasts(
    ...
    def _f(v: object) -> float | None:
        """Convert to float, mapping NaN → None so sqlite3 stores SQL NULL."""
    ...
    for _, row in combined_df.iterrows():
        ...
        records.append((
            ...
            _f(row["point_forecast"]),
            _f(row["p10"]), _f(row["p25"]), _f(row["p50"]),
            _f(row["p75"]), _f(row["p90"]),
        ))
    conn.executemany(
...
def run_hybrid_model(
    ...
    regime = get_current_regime(conn, max_date=max_date)
