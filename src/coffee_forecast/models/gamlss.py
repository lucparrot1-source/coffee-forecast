...
def compute_regime_labels(
    ...
    thresholds computed over the full available history up to max_date.
    ...
    df = pd.read_sql(
        f"SELECT date, adj_close FROM prices_monthly WHERE symbol = 'KC=F'{date_filter} ORDER BY date",
        conn,
        params=extra_param,
    )
    if df.empty:
...
def run_gamlss_model(
    conn: sqlite3.Connection, vecm_run_id: int, max_date: str | None = None
) -> int:
